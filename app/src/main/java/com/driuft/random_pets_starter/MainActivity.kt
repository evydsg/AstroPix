...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var nasaList: MutableList<Triple<String,String,String>>
    ...
    private fun getNasaImageURL() {
        ...
        client["https://api.nasa.gov/planetary/apod?api_key=YcWBOh5Le8HE1vHE4H7QBc9F3Ekzf7Hlu9GYzNQU&count=10", object : JsonHttpResponseHandler() {
            override fun onSuccess(statusCode: Int, headers: Headers, json: JSON) {
                ...
                val Nasadate = json.jsonArray.getJSONObject(0).getString("date")
                val NasaTitle = json.jsonArray.getJSONObject(0).getString("title")

                for(i in 0 until  NasaImage.length())
                {
                  val Nasaimage = json.jsonArray.getJSONObject(i).getString("url")
                    val Nasadate = json.jsonArray.getJSONObject(i).getString("date")
                        val NasaTitle = json.jsonArray.getJSONObject(i).getString("title")

                   nasaList.add(Triple(Nasaimage, Nasadate, NasaTitle))
                }
